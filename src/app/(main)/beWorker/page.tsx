...
import { CheckCircle2, ShieldCheck, MessageSquare, Wallet, Zap } from 'lucide-react';
    ...
    <div className="flex flex-col gap-0 bg-slate-950">
      ...
      <section className="relative py-24 lg:py-32 overflow-hidden min-h-[650px] flex items-center">
        ...
        <div className="container mx-auto px-4 relative z-10 text-center">
          <span className="inline-block py-1 px-3 rounded-full bg-[#d73cbe]/20 border border-[#d73cbe]/30 text-[#d73cbe] text-xs font-bold tracking-wider mb-6 backdrop-blur-sm">
          ...
          <p className="text-slate-200 text-lg md:text-xl max-w-2xl mx-auto mb-10 leading-relaxed drop-shadow-lg font-medium">
            O método mais seguro e descomplicado do mercado. Você foca no trabalho, 
            nós garantimos o pagamento e a segurança.
          </p>
          
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/login" className="px-8 py-4 rounded-full bg-[#d73cbe] hover:bg-[#c02aa8] text-white font-bold text-lg shadow-lg shadow-[#d73cbe]/25 transition-all hover:-translate-y-1">
              Começar Agora
            </Link>
            
            <Link href="#como-funciona" className="px-8 py-4 rounded-full bg-white/10 border border-white/20 hover:bg-white/20 text-white font-medium text-lg transition-all backdrop-blur-sm flex items-center justify-center">
              Entender o Processo
            </Link>
          </div>
        </div>
      ...
      <section id="como-funciona" className="py-20 bg-slate-900/50">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-bold text-white mb-4 font-futura">Como Funciona?</h2>
            <p className="text-slate-400">Sem burocracia desnecessária. Simples e direto.</p>
          </div>
        ...
        <div className="container mx-auto px-4 flex flex-col md:flex-row items-center gap-12">
          <div className="flex-1 space-y-6">
            ...
            <p className="text-slate-400 text-lg leading-relaxed">
              ...
              Acompanhe suas notas, feedbacks de clientes, faturamento mensal 
              e status de cada projeto em tempo real.
            </p>
            <ul className="space-y-3">
              <li className="flex items-center gap-3 text-slate-300">
                <CheckCircle2 className="text-[#d73cbe] w-5 h-5" /> Gestão de reputação (Avaliações)
              </li>
              <li className="flex items-center gap-3 text-slate-300">
                <CheckCircle2 className="text-[#d73cbe] w-5 h-5" /> Histórico financeiro detalhado
              </li>
              <li className="flex items-center gap-3 text-slate-300">
                <CheckCircle2 className="text-[#d73cbe] w-5 h-5" /> Chat centralizado
              </li>
            </ul>
          </div>
          <div className="flex-1 w-full h-64 md:h-96 rounded-2xl bg-gradient-to-br from-slate-900 to-slate-800 border border-white/10 flex items-center justify-center relative overflow-hidden group">
             ...
             <p className="text-slate-500 font-medium">Imagem do Dashboard do Profissional</p>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            ...
            <p className="text-slate-400 max-w-2xl mx-auto">
              Comece gratuitamente ou acelere seus resultados com nossos planos Pro.
            </p>
          </div>
...
function PlanCard({ title, price, period, features, isPopular, highlight, borderGlow, buttonColor }: any) {
  return (
    <div className={`relative p-8 rounded-3xl border flex flex-col h-full ${borderGlow ? 'border-[#d73cbe]/50 shadow-[0_0_30px_rgba(215,60,190,0.15)]' : 'border-white/10 bg-white/5'} ${isPopular ? 'bg-slate-900' : ''}`}>
      ...
      <div className="mb-6">
        ...
        <div className="flex items-end gap-1">
          <span className="text-4xl font-bold text-white font-futura">{price}</span>
      ...
      <button className={`w-full py-3 rounded-xl font-bold text-white transition-all shadow-lg ${buttonColor || 'bg-slate-800 hover:bg-slate-700'}`}>
        Escolher {title}
      </button>
    </div>
  );
